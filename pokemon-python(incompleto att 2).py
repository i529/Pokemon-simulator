...
if resposta_do_jogador2 == 1:
	...
	def batalha(pokemon_jogador, pokemon_adversario1):
		...
		#Caso o pokemon escolhi pelo usuario seja o Charmander, o mesmo tera Scratch e Growl como ataques.
		...
		while(not(life_pokemon_user == 0)) and (not(life_pokemon_adv == 0)):

			print(f'Escolha o ataque do seu pokemon, digite 1 para utilizar o primeiro ataque ou 2 para utilizar o segundo ataque!\n {Ataques} ')
			escolha = int(input())
			#Primeira escolha de ataque
			if escolha == 1:
				#Subtração da vida do pokemon adversário
				life_pokemon_adv = (life_pokemon_adv - 40)
				print(f"O {pokemon_adversario1} sofreu 40 de dano!")
				print(f"Vida do {pokemon_adversario1}:{life_pokemon_adv}")
			#Segunda escolha de ataque
			elif escolha == 2:
				#Subtração da vida do pokemon adversário
				life_pokemon_adv = (life_pokemon_adv - 0)
				print(f"O {pokemon_adversario1} sofreu 0 de dano e teve sua defesa reduzida!")
				print(f"vida do {pokemon_adversario1}:{life_pokemon_adv}")

			print("O adversário vai atacar...")
			#O oponente ataca
			Ataques_adversario1 = random.choice(Ataques_adversario1)
			if (Ataques_adversario1 == "Tackle") or (Ataques_adversario1 == "Scratch"):
				print(f'O {pokemon_adversario1} usou {Ataques_adversario1}!')
			...
			elif (Ataques_adversario1 == "Growl") or (Ataques_adversario1 == "Tail Whip"):
				print(f'O {pokemon_adversario1} usou {Ataques_adversario1}!')
				#subtração da vida do pokemon do usuário!
			...
			print('Aguardando a proxima ação do usuario.')
		if (life_pokemon_user == 0):
				print(f'O {adversario} venceu a batalha! -40 de pokemoney foi retirado do usuario!')
		elif(life_pokemon_adv == 0):
				print(f'O{usuario} venceu a batalha! -80 de pokemoney foi retirado do usuario!')
